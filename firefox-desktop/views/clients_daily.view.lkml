...
explore: clients_daily {
  ...
  join: clients_daily__experiments {
    view_label: "Clients Daily: Experiments"
    sql: LEFT JOIN UNNEST(${clients_daily.experiments}) as clients_daily__experiments ;;
    relationship: one_to_many
  }

  join: clients_daily__active_addons {
    view_label: "Clients Daily: Active Addons"
    sql: LEFT JOIN UNNEST(${clients_daily.active_addons}) as clients_daily__active_addons ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_app_locales {
    view_label: "Clients Daily: Environment Settings Intl App Locales"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_app_locales}) as clients_daily__environment_settings_intl_app_locales ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_system_locales {
    view_label: "Clients Daily: Environment Settings Intl System Locales"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_system_locales}) as clients_daily__environment_settings_intl_system_locales ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_accept_languages {
    view_label: "Clients Daily: Environment Settings Intl Accept Languages"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_accept_languages}) as clients_daily__environment_settings_intl_accept_languages ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_available_locales {
    view_label: "Clients Daily: Environment Settings Intl Available Locales"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_available_locales}) as clients_daily__environment_settings_intl_available_locales ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_requested_locales {
    view_label: "Clients Daily: Environment Settings Intl Requested Locales"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_requested_locales}) as clients_daily__environment_settings_intl_requested_locales ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_telemetry_event_counts_sum {
    view_label: "Clients Daily: Scalar Parent Telemetry Event Counts Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_telemetry_event_counts_sum}) as clients_daily__scalar_parent_telemetry_event_counts_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_typed_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Typed Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_typed_sum}) as clients_daily__scalar_parent_urlbar_searchmode_typed_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_other_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Other Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_other_sum}) as clients_daily__scalar_parent_urlbar_searchmode_other_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_content_telemetry_event_counts_sum {
    view_label: "Clients Daily: Scalar Content Telemetry Event Counts Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_content_telemetry_event_counts_sum}) as clients_daily__scalar_content_telemetry_event_counts_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_oneoff_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Oneoff Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_oneoff_sum}) as clients_daily__scalar_parent_urlbar_searchmode_oneoff_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_tabmenu_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Tabmenu Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_tabmenu_sum}) as clients_daily__scalar_parent_urlbar_searchmode_tabmenu_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_handoff_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Handoff Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_handoff_sum}) as clients_daily__scalar_parent_urlbar_searchmode_handoff_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__environment_settings_intl_regional_prefs_locales {
    view_label: "Clients Daily: Environment Settings Intl Regional Prefs Locales"
    sql: LEFT JOIN UNNEST(${clients_daily.environment_settings_intl_regional_prefs_locales}) as clients_daily__environment_settings_intl_regional_prefs_locales ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_touchbar_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Touchbar Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_touchbar_sum}) as clients_daily__scalar_parent_urlbar_searchmode_touchbar_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_shortcut_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Shortcut Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_shortcut_sum}) as clients_daily__scalar_parent_urlbar_searchmode_shortcut_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Tabtosearch Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_tabtosearch_sum}) as clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_bookmarkmenu_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Bookmarkmenu Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_bookmarkmenu_sum}) as clients_daily__scalar_parent_urlbar_searchmode_bookmarkmenu_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_keywordoffer_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Keywordoffer Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_keywordoffer_sum}) as clients_daily__scalar_parent_urlbar_searchmode_keywordoffer_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_topsites_newtab_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Topsites Newtab Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_topsites_newtab_sum}) as clients_daily__scalar_parent_urlbar_searchmode_topsites_newtab_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_topsites_urlbar_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Topsites Urlbar Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_topsites_urlbar_sum}) as clients_daily__scalar_parent_urlbar_searchmode_topsites_urlbar_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_onboard_sum {
    view_label: "Clients Daily: Scalar Parent Urlbar Searchmode Tabtosearch Onboard Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_urlbar_searchmode_tabtosearch_onboard_sum}) as clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_onboard_sum ;;
    relationship: one_to_many
  }

  join: clients_daily__scalar_parent_devtools_accessibility_select_accessible_for_node_sum {
    view_label: "Clients Daily: Scalar Parent Devtools Accessibility Select Accessible For Node Sum"
    sql: LEFT JOIN UNNEST(${clients_daily.scalar_parent_devtools_accessibility_select_accessible_for_node_sum}) as clients_daily__scalar_parent_devtools_accessibility_select_accessible_for_node_sum ;;
    relationship: one_to_many
  }
}

view: clients_daily {
  sql_table_name: `mozdata-nonprod.telemetry.clients_daily`
    ;;
...
view: clients_daily__experiments {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__active_addons {
  dimension: addon_id {
    type: string
    sql: ${TABLE}.addon_id ;;
  }
...
view: clients_daily__environment_settings_intl_app_locales {
  dimension: clients_daily__environment_settings_intl_app_locales {
    type: string
    sql: clients_daily__environment_settings_intl_app_locales ;;
  }
}

view: clients_daily__environment_settings_intl_system_locales {
  dimension: clients_daily__environment_settings_intl_system_locales {
    type: string
    sql: clients_daily__environment_settings_intl_system_locales ;;
  }
}

view: clients_daily__environment_settings_intl_accept_languages {
  dimension: clients_daily__environment_settings_intl_accept_languages {
    type: string
    sql: clients_daily__environment_settings_intl_accept_languages ;;
  }
}

view: clients_daily__environment_settings_intl_available_locales {
  dimension: clients_daily__environment_settings_intl_available_locales {
    type: string
    sql: clients_daily__environment_settings_intl_available_locales ;;
  }
}

view: clients_daily__environment_settings_intl_requested_locales {
  dimension: clients_daily__environment_settings_intl_requested_locales {
    type: string
    sql: clients_daily__environment_settings_intl_requested_locales ;;
  }
}

view: clients_daily__scalar_parent_telemetry_event_counts_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_typed_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_other_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_content_telemetry_event_counts_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_oneoff_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_tabmenu_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_handoff_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__environment_settings_intl_regional_prefs_locales {
  dimension: clients_daily__environment_settings_intl_regional_prefs_locales {
    type: string
    sql: clients_daily__environment_settings_intl_regional_prefs_locales ;;
  }
}

view: clients_daily__scalar_parent_urlbar_searchmode_touchbar_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_shortcut_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_bookmarkmenu_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_keywordoffer_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_topsites_newtab_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_topsites_urlbar_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_urlbar_searchmode_tabtosearch_onboard_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
...
view: clients_daily__scalar_parent_devtools_accessibility_select_accessible_for_node_sum {
  dimension: key {
    type: string
    sql: ${TABLE}.key ;;
  }
